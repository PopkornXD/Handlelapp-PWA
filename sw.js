...
self.addEventListener('fetch', event => {
  ...
  if (req.mode === 'navigate' || (req.headers.get('accept') || '').includes('text/html')) {
    event.respondWith((async () => {
      ...
      try {
        ...
        const isRedirect = net && (net.type === 'opaqueredirect' || net.redirected || (net.status >= 300 && net.status < 400));
        if (!isRedirect && net && net.ok) {
  ...
  event.respondWith((async () => {
    ...
    try {
      ...
      const isRedirect = net && (net.type === 'opaqueredirect' || net.redirected || (net.status >= 300 && net.status < 400));
      if (!isRedirect && net && net.ok) {
        try { await cache.put(req, net.clone()); } catch (e) { console.warn('[sw] put asset failed', e); }
      }
      return net;
    } catch (e) {
